...
class _SlidingUpPanelState extends State<SlidingUpPanel> with SingleTickerProviderStateMixin {
                ...
                child: AnimatedBuilder(
                  ...
                  builder: (context, _) {
                    return Container(
                      ...
                      // Set color to null so that touch events pass through to the body when the panel is closed,
                      // otherwise, if a color exists, then touch events won't go through.
                      color: _ac.value == 0.0
                          ? null
  ...
  // Returns a gesture detector if panel is used and a listener if panelBuilder is used. This is because the listener
  // is designed only for use with linking the scrolling of panels and using it for panels that don't want to linked
  ...
  void _onGestureSlide(double dy) {
    ...
    // If the panel is open and the user hasn't scrolled, we need to determine whether to enable scrolling if the user
    // swipes up, or disable closing and begin to close the panel if the user swipes down.
  ...
  void _onGestureEnd(Velocity v) {
    ...
    // If scrolling is allowed and the panel is open, we don't want to close the panel if they swipe up on the
    // scrollable.
  ...
  // Animate the panel position to the snap point. REQUIRES that widget.snapPoint != null.
  ...
  // Set the panel position to value - must be between 0.0 and 1.0
  set _panelPosition(double value) {
    assert(0.0 <= value && value <= 1.0);
    _ac.value = value;
  }

  // Get the current panel position, returns the % offset from collapsed state as a decimal between 0.0 and 1.0.
